...
function toUserList(u) {
	...
	for (var i = 0; i < us.length; i++) {
		...
		var btn = window.document.querySelector('button._5f5mN._6VtSN.yZn4P');
...
function klik(b) {
	...
	waits(1,1);
	iimDisplay(act + ' ' + n);
	window.document.querySelector('button.-Cab_').click();
	waits(min,max);
	var btn = window.document.querySelector('button._5f5mN._6VtSN.yZn4P');
	var btnPvt = window.document.querySelector('button.BY3EC.sqdOP.L3NKy.y3zKF');
	if (btn&&btn.innerText==act) {
	...
	do{
		...
		for (i=0;i<btn.length;i++){
			if (btn[i].innerText == act){
				...
				if (btn[i].innerText == act){
					countdown(910,'MENTAL',n);
				}else if (btn[i].innerText == req && skipPrivate) {
					n -= 1;
					btn[i].click();
					iimDisplay(act + ' ' + n);
					waits(1,1);
...
function showPanel(){
	try{
		...
		var newStyle;
            newStyle = 'div.modal__content{left:0;bottom:60px; position:fixed; border-radius:5px; background-color:#fff; margin:5px; width:300px; height:475px; box-shadow:0 4px 8px 0 rgba(0,0,0,0.2),0 6px 20px 0 rgba(0,0,0,0.19); z-index: 2;}';
		...
		var modal = '<div id="modalPlain" class="" role="modal"><div class="modal__content"><div class="modal__header"><h2><span>Gramatic 3.1</span></h2><span class="modal__close" title="ESCAPE to close"></span></div>';
            ...
            modal += '<input type="number" name="maxfollow" id="maxfollow" value=10 min="0" title="Jumlah Maksimal" style="display:none;"><span title="" style="font-weight: normal; display:none;">user</span><span class="spCnt" style="display: flex;">Delay action:</span><label for="min">delay min</label>';
            ...
            modal += '<label for="delay2">delay time</label><input type="number" name="delay2" id="delay2" value=900 min="1" ><span style="font-weight: normal;">seconds</span><br/><label for="delayakun">delay max</label><input type="number" name="delayakun" id="delayakun" value=15 title="Maksimal 15 User"><span style="font-weight: normal;">users</span><span class="spCnt" style="display: flex;">Delay session 2</span>';
