...
function userAgent(){
	...
	window.resizeTo(800, (window.screen.height-40));
}
function getlinkpost(){
			...
			while(links.length < maxaction)	{
				...
				if (isgrP) {
					...
					for (j=0; j < listlink.length;j++){
						var link = listlink[j];
						if (link.hasAttribute('title')&&!links.includes(link.title)) {
	...
	for (f=0; f<links.length;f++){
		iimDisplay('DM ' + parseInt(f+1));
		openpost(links[f],lang,op);
		if (f%delayakun == parseInt(delayakun-1)&&f>0){
			countdown(delay2,f);
		}else{
			random(min,max);
		}
...
function openpost(a,l,o) {
	if (l = 'en'){
		...
		var sorryText = 'Maaf, halaman ini tidak tersedia';
	...
	if (o == 2 || o == 3){
		...
		alert('Auto DM tidak berjalan di halaman ini.');
	}
	function gonext() {
		comments(a,commentval,l,jac);
	}
}

function comments(u,comm,lang,jac) {
	var cvl = comm.replace(/\[username\]/g,u).replace(/ /g,'<sp>').replace(/\n/g,'<br>');
	if (lang = 'en'){var sendText = 'Send';var msgText = 'Message';var followText = 'Follow';var unlikeText = 'Unlike';var likeText = 'Like'; var commentText = 'Comment';var postText = 'Post';var noPostText = 'No Posts Yet';var privateText='This Account is Private';}else{var sendText = 'Kirim*';var msgText = 'Kirim*pesan';var followText = 'Ikuti';var unlikeText = 'Batal*Suka';var privateText='Akun Ini Bersifat Pribadi'; var noPostText ='Belum Ada Postingan'; var postText ='Kirim'; var commentText='Komentari'; var likeText='Suka';}
	if (jac=='200'){
		var fl = 'CODE:';
	...
	var un = 'CODE:';
		...
		un +='TAG POS=1 TYPE=BUTTON ATTR=TXT:' + msgText + ' EXTRACT=TXT\n';
	iimPlay(un);
	var msgok = iimGetLastExtract();
	if (msgok != '#EANF#'){
		var li = 'CODE:';
			li +='SET !TIMEOUT_STEP 1\n';
			li +='SET !ERRORIGNORE YES\n';
			li +='TAG POS=1 TYPE=BUTTON ATTR=TXT:' + msgText + '\n';
			li +='WAIT SECONDS=3\n';
			li +='TAG POS=1 TYPE=TEXTAREA ATTR=PLACEHOLDER:*...&&TXT: CONTENT=' + cvl + '\n';
			li +='WAIT SECONDS=3\n';
			li +='TAG POS=1 TYPE=BUTTON ATTR=TXT:' + sendText + '\n';
			li +='WAIT SECONDS=5\n';
			li +='TAB CLOSE\n';
		iimPlay(li);
	}else{
...
function countdown(loop,f){
	for(s=loop;s>0;s--){
		...
		var prosesval = 'Next DM After ' + Math.floor(menit) + ' Minutes ' + Math.floor(detik) + ' Seconds';
		iimDisplay('DM ' + parseInt(f+1) + '\n\nAuto DM Akan Dilanjutkan Setelah: \n' + Math.floor(menit) + ' Menit ' + Math.floor(detik) + ' Detik.');
...
function openModal(){
	try{
		...
		var modal  = '<div id="modalPlain" class="" role="modal"><div class="modal__content"><div class="modal__header"><h2><span>Gramatic 3.1</span></h2><span class="modal__close" title="ESCAPE to close"></span></div>';
			modal += '<div class="modal__body"><fieldset class="modal-grm" id="col-setting"><div class="modal-grm" id="btn-setting"></div><div class="pop-comment-menu" id="pop-c" style="display:none;"><div class="pop-carret"></div><div class="pop-comment-container"><p class="notess">Follow maksimal 40 akun tiap session dengan jeda minimal 1 detik & <strong>delay (minimal) 10 menit</strong> untuk follow 40 akun berikutnya.</p></div></div><span style="display: inline-block;width:40%;margin:0 10px 0 0;">Jenis DM</span><select id="jenisaction" onchange="ja()"><option value="100">DM Following</option> <option value="200">Follow + DM</option></select><span style="display: flex;">Jumlah DM</span><label for="maxaction">Maksimal</label><input type="number" name="maxaction" id="maxaction" value=10 min=1 title="Jumlah Maksimal"><span style="font-weight: normal;">User</span><span style="display: flex;">Delay Time (Random):</span><label for="min">Minimal</label><input type="number" name="min" id="min" value=10 min=1><span style="font-weight: normal;">Detik</span><br/><label for="max">Maksimal</label><input type="number" name="max" id="max" min=1 value=20><span style="font-weight: normal;">Detik</span><span style="display: flex;">Delay Session</span><label for="delay2" title="Minimal 600 Detik (10 Menit)">Delay Time</label><input type="number" name="delay2" id="delay2" value=600 min=60><span style="font-weight: normal;">Detik</span><br/><label for="delayakun" title="Maksimal 40 User">Maksimal</label><input type="number" name="delayakun" id="delayakun" value=10 max=40 title="Maksimal 40 User"><span title="Maksimal 40 User" style="font-weight: normal;">User</span></fieldset><div id="komen" style="display: none;"><span>Comment Text</span><div class="text-wrapper"><textarea id="komentar" rows="12" title="Use Your Best Spintext to Make\'s it Unix." placeholder="Add a comment... (support nested spintax)"></textarea><textarea id="hslkomentar" disabled="" style="display: none;"></textarea></div></div>';
